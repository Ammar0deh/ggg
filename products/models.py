...
class Order(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.paid and self.sended and not self.closed:
            ...
            # Create an invoice when the order is closed
            invoice = Invoice.objects.create(order=self)
            # You can set the payment method for the invoice, e.g., 'Credit Card' or 'Fake Payment'
            invoice.payment_method = 'Fake Payment'  # Change this to the actual payment method used
            invoice.amount_paid = self.total  # Assuming you have a total property in the Order model
            invoice.save()
        super().save(*args, **kwargs)
...
class Invoice(models.Model):
    ...
    def __str__(self):
        return f'Invoice for Order - {self.order}'
